                                 ...
                                 cbc_opcode opcode) /**< opcode */
{
  if (context_p->last_cbc_opcode == CBC_PUSH_IDENT)
  {
    ...
    {
      parser_raise_error (context_p, "Eval or arguments cannot be assigned in strict mode.");
    }
 ...
 */
static void
parser_parse_array_literal (parser_context *context_p) /**< context */
{
  int pushed_items = 0;
  ...
  while (PARSER_TRUE)
  {
    ...
    else
    {
      ...
      {
        parser_raise_error (context_p, "Expected ',' or ']' after an array item.");
      }
    }
 ...
 */
static void
parser_parse_object_literal (parser_context *context_p) /**< context */
{
  ...
  while (PARSER_TRUE)
  {
    ...
    {
      uint16_t literal_index = context_p->literal_object.index;

      lexer_next_token (context_p);
      if (context_p->token.type != LEXER_COLON)
      {
        parser_raise_error (context_p, "Expected ':' after property identifier.");
      }
    ...
    {
      parser_raise_error (context_p, "Expected ',' or '}' after a property definition.");
    }
  }
                               ...
                               size_t *grouping_level_p) /**< grouping level */
{
  ...
  switch (context_p->token.type)
  {
    case LEXER_LITERAL:
    {
          ...
          || context_p->token.literal_location.type == LEXER_STRING_LITERAL)
      {
        lexer_construct_literal_object (context_p,
                                        ...
                                        context_p->token.literal_location.type);
      }
      else if (context_p->token.literal_location.type == LEXER_NUMBER_LITERAL)
      {
    ...
    {
      lexer_construct_regexp_object (context_p);
