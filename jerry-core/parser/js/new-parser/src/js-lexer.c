 ...
 */
static void
lexer_parse_identifier (parser_context *context_p, /**< context */
                        int check_keywords) /**< check keywords */
{
  ...
  context_p->token.literal_location.has_escape = PARSER_FALSE;
  ...
  do
  {
    if (*source_p == '\\')
    {
      ...
      context_p->token.literal_location.has_escape = PARSER_TRUE;
  ...
  if (check_keywords
      && !context_p->token.literal_location.has_escape
      && (length >= 2 && length <= 10))
  {
  ...
  {
    /* Fill literal data. */
    context_p->token.literal_location.char_p = ident_start_p;
    context_p->token.literal_location.length = length;
  }
...
lexer_parse_string (parser_context *context_p) /**< context */
{
  ...
  /* Fill literal data. */
  context_p->token.literal_location.char_p = string_start_p;
  context_p->token.literal_location.length = length;
  context_p->token.literal_location.type = LEXER_STRING_LITERAL;
  context_p->token.literal_location.has_escape = has_escape;
 ...
 */
static int
lexer_is_hex_number (uint8_t character)
{
  return (character >= '0' && character <= '9')
          || ((character | 0x20) >= 'a' && (character | 0x20) <= 'f');
} /* lexer_is_hex_number */
 ...
 */
static void
lexer_parse_number (parser_context *context_p) /**< context */
{
  const uint8_t *source_p = context_p->source_p;
  const uint8_t *source_end_p = context_p->source_end_p;
  ...
  context_p->token.type = LEXER_LITERAL;
  context_p->token.literal_location.char_p = source_p;
  context_p->token.literal_is_reserved = PARSER_FALSE;
  context_p->token.literal_location.type = LEXER_NUMBER_LITERAL;
  context_p->token.literal_location.has_escape = PARSER_FALSE;
      ...
      && (source_p[1] | 0x20) == 'x')
  {
    ...
    if (source_p >= source_end_p
        || !lexer_is_hex_number (source_p[0]))
    {
      parser_raise_error (context_p, "Invalid hexadecimal number.");
    }
    ...
    while (source_p < source_end_p
           ...
           && source_p[1] <= '9')
  {
    parser_raise_error (context_p, "Invalid number.");
  }
  else
  {
    while (source_p < source_end_p
        ...
        && (source_p[0] | 0x20) == 'e')
    {
      ...
      if (source_p >= source_end_p
          ...
          || source_p[0] > '9')
      {
        parser_raise_error (context_p, "Missing exponent number.");
      }
  ...
  {
    parser_raise_error (context_p, "Identifier cannot start after a number");
  }

  length = source_p - context_p->source_p;
  if (length > PARSER_MAXIMUM_IDENT_LENGTH)
  {
    parser_raise_error (context_p, "Number too long.");
  }

  context_p->token.literal_location.length = (uint16_t) length;
  context_p->column += (lexer_line_counter) length;
  context_p->source_p = source_p;
} /* lexer_parse_number */
                            ...
                            uint8_t literal_type) /**< final literal type */
{
  ...
  while ((literal_p = (lexer_literal *) parser_list_iterator_next (&literal_iterator)) != NULL)
  {
    ...
    {
      context_p->literal_object.literal_p = literal_p;
      context_p->literal_object.index = (uint16_t) literal_index;
      return;
    }
    literal_index++;
  }
...
lexer_construct_literal_object (parser_context *context_p, /**< context */
                                lexer_literal_location *literal_p, /**< literal location */
                                uint8_t literal_type) /**< final literal type */
{
  ...
  if (literal_p->type == LEXER_IDENT_LITERAL)
  {
    ...
    {
      context_p->literal_object.type = lexer_literal_object_eval;
    }
    ...
    {
      context_p->literal_object.type = lexer_literal_object_arguments;
    }
  }
 ...
 */
void
lexer_construct_number_object (parser_context *context_p) /**< context */
{
  lexer_literal *literal_p;
  ...
  literal_p = (lexer_literal *) parser_list_append (context_p, &context_p->literal_pool);
  literal_p->length = context_p->token.literal_location.length;
  literal_p->type = LEXER_UNKNOWN_LITERAL;
  literal_p->status_flags = 0;

  context_p->literal_count = literal_count + 1;

  if (util_set_number_literal (literal_p, context_p->token.literal_location.char_p))
  {
    parser_raise_error (context_p, "Out of memory.");
  }
  ...
  context_p->literal_object.literal_p = literal_p;
  context_p->literal_object.index = literal_count;
  context_p->literal_object.type = lexer_literal_object_any;
 ...
 */
void
lexer_construct_function_object (parser_context *context_p, /**< context */
 ...
 */
void
lexer_construct_regexp_object (parser_context *context_p) /**< context */
{
  lexer_literal *literal_p;
  const uint8_t *regexp_end_p;
  size_t offset;
  ...
  regexp_end_p = util_set_regexp_literal (literal_p,
                                          context_p->source_p - offset,
                                          context_p->source_end_p);

  if (regexp_end_p == NULL)
  {
    parser_raise_error (context_p, "Invalid regular expression.");
  }

  context_p->column += regexp_end_p - context_p->source_p;
  context_p->source_p = regexp_end_p;

  literal_p->type = LEXER_REGEXP_LITERAL;

  context_p->token.type = LEXER_LITERAL;
  context_p->token.literal_is_reserved = PARSER_FALSE;
  context_p->token.literal_location.type = LEXER_REGEXP_LITERAL;

  context_p->literal_object.literal_p = literal_p;
  context_p->literal_object.index = context_p->literal_count - 1;
  context_p->literal_object.type = lexer_literal_object_any;
      ...
      && (util_is_identifier_start (context_p->source_p) || context_p->source_p[0] == '\\'))
  {
    ...
    if (context_p->token.type == LEXER_LITERAL)
    {
          ...
          && context_p->literal_object.type != lexer_literal_object_any)
      {
        ...
        if (context_p->literal_object.type == lexer_literal_object_eval)
        {
          error_message = "Eval is not allowed here in strict mode.";
        }
        else
        {
          PARSER_ASSERT (context_p->literal_object.type == lexer_literal_object_arguments);
          error_message = "Arguments is not allowed here in strict mode.";
        }

        parser_raise_error (context_p, error_message);
      }

      context_p->token.literal_location.type = literal_type;
      return;
    }
  }
...
static lexer_literal_location lexer_get_literal =
{
  (const uint8_t *) "get", 3, LEXER_IDENT_LITERAL, PARSER_FALSE
};

static lexer_literal_location lexer_set_literal =
{
 ...
 */
void
lexer_expect_object_literal_id (parser_context *context_p, /**< context */
                                int must_be_identifier) /**< only identifiers are accepted */
{
  ...
  if (context_p->source_p < context_p->source_end_p)
  {
    ...
    if (util_is_identifier_start (context_p->source_p) || context_p->source_p[0] == '\\')
    {
      ...
      if (!must_be_identifier
          && context_p->token.literal_location.length == 3)
      {
        lexer_token_type type = LEXER_EOS;

        if (lexer_same_identifiers (&context_p->token.literal_location, &lexer_get_literal))
        {
          type = LEXER_PROPERTY_GETTER;
        }
        else if (lexer_same_identifiers (&context_p->token.literal_location, &lexer_set_literal))
        {
          type = LEXER_PROPERTY_SETTER;
        }
  ...
  parser_raise_error (context_p, "Property identifier expected.");
 ...
 */
int
lexer_same_identifiers (lexer_literal_location *left, /**< left identifier */
                        lexer_literal_location *right) /**< right identifier */
{
